...
ipca_2023 <- c(-0.49)
sinapi_2023 <- c(1678.70, 1684.29, 1697.13, 1703.47, 1727.96, 1728.72, 1730.29, 1731.46, 1734.32)
fipezap_2023 <- c(124.40, 126.28, 128.88, 130.78, 132.73, 134.13, 135.56, 136.95, 138.07)
ipca_2024 <- c()
sinapi_2024 <- c(1735.48, 1735.65, 1743.89, 1744.18, 1769.02, 1769.19, 1778.76, 1778.27, 1778.27)
fipezap_2023 <- c(143.63, 146.08, 151.14, 157.64, 164.06, 168.36, 170.58, 172.82, 172.82)
```
...
-   A mediana do preço unitário dos aluguéis é maior em 2024 do que em 2023 para todas as cidades, exceto Esteio.
-   Em Esteio houve uma diminuição de imóveis em pontos altos da cidade.
:::
...
Na imagem abaixo vemos o leito do Rio dos Sinos em até 10 metros de altura.
...
- Há uma boa distribuição em imóveis que estão em áreas de risco e em áreas seguras.
:::
...
ggpairs(rs2023 %>% select(-data_inclusao), mapping = aes(color = cidade, alpha = 0.4)) +
  theme.base +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

ggpairs(rs2024 %>% select(-data_inclusao), mapping = aes(color = cidade, alpha = 0.4)) +
  theme.base +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
```
...
- O preço unitário dos aluguéis aumentou em 2024 em relação a 2023.
- Houve um aumento de variabilidade nos preços em 2024.
:::
